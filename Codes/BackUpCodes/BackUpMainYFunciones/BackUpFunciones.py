    ...
    for title in image_title:
        for iter,index in enumerate(ID): #tengo que iterar dentro del diccionario para poder coger tambien las coordenadas al mismo tiempo
            if(int(title[6:10]) == int(float(index))):
                ...
                if title in titlesn:
                    final_dic[title]['boxes'].append([x1[iter],y1[iter],x2[iter],y2[iter]]) #CAMBIAR TITLE por IMAGE_PATH URGENTEEEEEEEEEE
                else:
                    titlesn.append(title) #dejar la ruta desde castings
                    final_dic[title] = {'w': data_image1.width,'h': data_image1.height,'boxes': [[x1[iter],y1[iter],x2[iter],y2[iter]]]} #Añadir un diccionario en title 
...
def reading_train_test (final_dic):
    ...
    for j in image_title_train:
        if j in final_dic:
            print("Information about " + str(j) + " --> " + str(final_dic[j]))
